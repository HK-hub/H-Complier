...
//        Queue<Closure> queue = new LinkedList<>();
    ...
    public ALGraph getInverseGraph(){
        if(this.type==GraphType.digraph || this.type==GraphType.dinetwork){
            ...
            for (VexNode vexNode : this.adjlist) {
                newadjlist.add(new VexNode(vexNode.data, null));
            }
    ...
    private void explore(int[] prev, int[] post, int i) {
        prev[i] = clock++;
        EdgeNode edge = adjlist.get(i).firstedge;
        while (edge!=null){
    ...
    public class EdgeNode implements Serializable{
        ...
        public boolean equals(Object o) {
            ...
            if (adjvex != edgeNode.adjvex) {
                return false;
            }
            if (weight != edgeNode.weight) {
                return false;
            }
            if(nextedge!=null) {
